...
public static class ForbidLessThanOrEqualExtensions
{
    ...
    public static T LessThanOrEqual<T>(this IForbid forbid, T input, T valueToCompare)
        where T : struct, IComparable<T> => LessThanOrEqual(input, valueToCompare);
    ...
    public static T LessThanOrEqual<T>(this IForbid forbid, T input, T valueToCompare, string message)
        where T : struct, IComparable<T> => LessThanOrEqual(input, valueToCompare, message);
    ...
    public static T LessThanOrEqual<T>(this IForbid forbid, T input, T valueToCompare, Exception exception)
        where T : struct, IComparable<T> => LessThanOrEqual(input, valueToCompare, null, exception);
    ...
    /// <typeparam name="T">Any type of object.</typeparam>
    /// <returns><see cref="T"/> input.</returns>
    private static T LessThanOrEqual<T>(T x, T y, string? message = null, Exception? exception = null)
        where T : struct, IComparable<T>
    {
        Forbid.From.Null(x);
        Forbid.From.Null(y);
        ForbidDefaultExtensions.Equal(x, y, message, exception);
        if (x.CompareTo(y) < 0)
